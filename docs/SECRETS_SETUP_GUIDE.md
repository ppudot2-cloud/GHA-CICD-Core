...
| Secret | Description |
|---|---|
| `GHA_CORE_PAT` | Personal Access Token (or GitHub App token) with `repo` scope. Used to check out the private GHA-CICD-Core repository on runners and to create pull requests via `gh pr create`. Prefer a GitHub App over a PAT in production. |
